...
public class ODataApikitRequestTestCase extends MuleArtifactFunctionalTestCase
{
    ...
    public void getOrdersWithAllpagesInlineCount() throws Exception
    {
        given().header("Accept", "application/json")
                .expect()
                .response()
                .body("d.__count", equalTo("2"))
                .header("Content-type", "application/json").statusCode(200)
                .when().get("api/odata.svc/orders?$format=json&$inlinecount=allpages").thenReturn();
    }
    ...
    public void getOrdersWithNoneInlineCount() throws Exception
    {
        given().header("Accept", "application/json")
                ...
                .body("d.__count", nullValue())
                .header("Content-type", "application/json").statusCode(200)
                .when().get("api/odata.svc/orders?$format=json&$inlinecount=none").thenReturn();
    }
    ...
    {
    	given().header("Accept", "application/json")
    	.body("{\"OrderID\":20248,\"ShipAddress\": null,\"ShipName\":\"ship-name\"}")
        .expect()
        	.statusCode(200)
        .when().post("api/odata.svc/orders");
    }
    ...
    {
    	given().header("Accept", "application/json")
    	.body("{\"OrderID\":20000,\"ShipName\":\"ship-name\"}")
        .expect()
        	.statusCode(200)
        .when().post("api/odata.svc/orders");
    }
    ...
    {
        given().header("Accept", "application/json")
                .body("{\"OrderID\":20000,\"ShipAddress\":\"ship-address-1\",\"ShipAddress\":\"ship-address-1\",\"ShipName\":\"ship-name-1\"}")
                .expect()
                .statusCode(400)
                .when().post("api/odata.svc/orders");
    }
