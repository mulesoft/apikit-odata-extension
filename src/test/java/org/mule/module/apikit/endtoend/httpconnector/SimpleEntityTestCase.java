...
public class SimpleEntityTestCase extends MuleArtifactFunctionalTestCase {
  ...
  public void getOrdersPositive() throws Exception {
    given().header("Accept", "application/json").expect().statusCode(200).when()
    ...
    given().expect().response().body("d.results.OrderID", hasItems(10248, 10249))
        .header("Content-type", "application/json").statusCode(200).when()
        .get("api/odata.svc/orders?$format=json");
  }
  ...
  public void gettingNonExistentEntityShouldResponse404Code() throws Exception {
    given().header("Accept", "application/json").expect().header("Content-type", "application/json")
        .statusCode(404).when().get("api/odata.svc/nonExistentProperty");
  }
  ...
  public void countCollections() throws Exception {
    ...
    given().header("Accept", "application/json").expect().response()
        .body("\"odata.error\".message.value", containsString("Unsupported media type requested"))
        .when().get("api/odata.svc/orders/$count");

    // send the right MIME-type in the header
    given().header("Accept", "text/plain").expect().response().body(equalTo("2"))
        .header("Content-Type", containsString("text/plain")).when()
        .get("api/odata.svc/orders/$count");

    // send no headers at all
    given().expect().response().body(equalTo("2"))
        .header("Content-Type", containsString("text/plain")).when()
        .get("api/odata.svc/orders/$count");
  ...
  public void getOrderPositive() throws Exception {
    given().header("Accept", "application/json").expect().response()
        .body("d.results.__metadata.uri",
            hasItems("http://127.0.0.1:" + serverPort.getNumber() + "/api/odata.svc/orders(10248)"))
        .header("Content-type", "application/json").statusCode(200).when()
