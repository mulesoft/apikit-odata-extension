...
public class MetadataTestCase extends MuleArtifactFunctionalTestCase {
  ...
  public void metadataPositive() throws Exception {
    given().header("Accept", "application/xml").expect().response()
        .body("Edmx.DataServices.Schema.EntityType.@Name", hasItems("orders"))
        .header("Content-type", "application/xml").statusCode(200).when()
        .get("/api/odata.svc/$metadata");
  }

  @Test
  public void metadataJsonNegativePut() {
    given().header("Accept", "application/json").expect().statusCode(405).when()
        .put("/api/odata.svc/$metadata");
  }

  @Test
  public void metadataJsonNegativeDelete() {
    given().header("Accept", "application/json").expect().statusCode(405).when()
        .delete("/api/odata.svc/$metadata");
  }

  @Test
  public void metadataXmlNegativePost() {
    given().header("Accept", "application/xml").expect().statusCode(405).when()
        .post("/api/odata.svc/$metadata");
  }
