...
public class ODataUriHelper {
  ...
  public static final String[] ODATA_VALID_PARAMS =
      {"$orderby", "$top", "$skip", "$filter", "$expand", "$format", "$select", "$inlinecount"};
  public static final String[] ODATA_LIST_PARAMS = {"$orderby", "$inlinecount", "$skip", "$top"};
  public static final String ODATA_RESOURCE_REGEXP =
      "^/((\\w|\\-)+)(\\(((\\w|\\-)+|'[^']*'|(,?(\\w|\\-)+=((\\w|\\-)+|'[^']*'))+)\\))?(/|/\\$(\\w|\\-)+)?$";
  ...
  public static final String ODATA_MULTIPLE_KEYS_REGEXP =
      "^(((\\w|\\-)+)=((\\w|\\-)+|'[^']*'),?)+$";
  public static final String ODATA_ORDERBY_REGEXP =
      "^((\\w|\\-)+(/\\w)?(\\s(desc|asc))?(,\\s*(\\w|\\-)+(/\\w)?(\\s(desc|asc))?)*)+$";
  ...
  public static final Pattern ODATA_MULTIPLE_KEYS_PATTERN =
      Pattern.compile(ODATA_MULTIPLE_KEYS_REGEXP);
  ...
  public static HashMap<String, Object> parseKeys(String path, String[] entityKeys)
    ...
    if (!isNullOrEmpty(parsedId)) {
      if (matchPattern(ODATA_MULTIPLE_KEYS_PATTERN, parsedId)) {
        ...
        throw new ODataInvalidUriException("Invalid format for key value '" + parsedId
            + "' should be an int or a string wrapped in single quotes.");
      }
    } else {
        ...
        if (id.length == 1) // id isn't a key-value pair
        {
          ...
          if (key.equals(entityKeyName)) {
            ...
            throw new ODataInvalidUriException(
                "Invalid key '" + key + "' should be '" + entityKeyName + "'.");
          }
        }
      } else if (parsedKeys.length > 1) {
        throw new ODataInvalidUriException("Unexpected number of keys: found " + parsedKeys.length
            + " where " + entityKeys.length + " were expected.");
      }
      ...
      for (String queryParam : queryParams) {
        ...
        if (argument.startsWith("$")) {
          ...
          if (argument.equals("$format")) {
            ...
            throw new ODataInvalidUriException("The query parameter '" + argument
  ...
  public static boolean validArgumentValue(String argument, String value)
    ...
    switch (argument) {
      case "$orderby":
        ...
        if (!m.matches())
          throw new ODataInvalidUriException(
              "Incorrect format for $orderby argument '" + value + "'.");
        break;
      ...
      case "$skip":
        ...
        if (!m.matches())
          throw new ODataInvalidUriException(
              "Incorrect format for $skip argument '" + value + "'.");
        break;
      ...
      case "$expand":
        ...
        if (!m.matches())
          throw new ODataInvalidUriException(
              "Incorrect format for $expand argument '" + value + "'.");
        break;

      case "$format":
        m = ODATA_FORMAT_PATTERN.matcher(value);
        if (!m.matches())
          throw new ODataInvalidFormatException(
              "Incorrect format for $format argument '" + value + "'.");
        break;
      ...
      case "$inlinecount":
        ...
        if (!m.matches())
          throw new ODataInvalidUriException(
              "Incorrect format for $inlinecount argument '" + value + "'.");
        break;
