...
public class Helper {
  ...
  public static EntitiesResponse convertEntriesToOEntries(List<Entry> outputEntries,
      String entitySetName, EntityDefinitionSet metadata1, Integer inlineCount) {
    ...
    for (Entry outputEntry : outputEntries) {
      ...
      for (EdmProperty edmProperty : ees.getType().getProperties().toList()) {
        ...
        if (propertyValue != null) {
          ...
          OProperty oProperty =
              EDMTypeConverter.getOProperty(propertyName, propertyValue, edmProperty.getType());
    ...
    try {
      ...
      for (EntityDefinition entityMetadata : metadata.toList()) {
        ...
        for (EntityDefinitionProperty propertyMetadata : entityMetadata.getProperties()) {
          Builder builder = EdmProperty.newBuilder(propertyMetadata.getName())
              .setType(EDMTypeConverter.convert(propertyMetadata.getType()));
        ...
        EdmEntityType.Builder type = EdmEntityType.newBuilder().setNamespace(namespace)
            .setName(entityName).addKeys(keys).addProperties(properties);
        entityTypes.add(type);

        entitySets
            .add(EdmEntitySet.newBuilder().setName(pluralizeName(entityName)).setEntityType(type));
      }

      EdmEntityContainer.Builder container = EdmEntityContainer.newBuilder()
          .setName("ODataEntityContainer").setIsDefault(true).addEntitySets(entitySets);
      EdmSchema.Builder containerSchema = EdmSchema.newBuilder().setNamespace("odata2.namespace")
          .addEntityContainers(container).addEntityTypes(entityTypes);
